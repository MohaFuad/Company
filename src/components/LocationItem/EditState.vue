...
export default {
  ...
  methods: {
    async Edit()
    {
      ...
      localStorage.clear();
      await axios
        .patch(`http://localhost:3000/api/States/${id}`,{
        name:this.state,
        })
        .then((res) => {
          console.log(res.status);
          alert("تم تعديل المحافظة بنجاح");
  ...
  created() {
    const id = JSON.parse(localStorage.getItem('id'));
    console.log(id);
    axios.get(`http://localhost:3000/api/States/${id}`).then((res) => {
      console.log(res);
      this.state = res.data.state.name;
